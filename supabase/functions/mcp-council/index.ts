...
const TOOL_MANIFEST_VERSION = "2026.09.01.1";
...
const mcpHandler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (method === "tools/call") {
      ...
      if (
        name === "world_read" || name === "world_write" || name === "registers_read" ||
        ...
        name === "session_ledger" ||
        name === "project_thread_post" || name === "project_thread_read"
        ...
        } else if (name === "project_thread_post") {
          // Client-supplied cid is ignored · p_cid comes from tenantCid().
          rpcName = "project_thread_post";
          params = {
            p_cid: worldCid,
            p_blueprint: str(args?.blueprint),
            p_content: typeof args?.content === "string" ? args.content : null,
            p_author: str(args?.author),
          };
        } else if (name === "project_thread_read") {
          rpcName = "project_thread_read";
          params = {
            p_cid: worldCid,
            p_blueprint: str(args?.blueprint),
            p_limit: num(args?.limit, 20),
          };
        ...
        } else if (name === "world_write") {
          // The single write path into the world registers. Client-supplied
          // cid is ignored outright: p_cid comes from tenantCid() above.
          rpcName = "cob_world_write";
          params = {
            p_cid: worldCid,
            p_kind: str(args?.kind),
            p_payload: args?.payload && typeof args.payload === "object" ? args.payload : null,
            p_actor: str(args?.actor) ?? "cob:world_write",
          };
        ...
        // ATTRIBUTION · actor/surface/model travel with the RPC request.
        const rpcClient = rpcClientFor(ATTRIBUTION.get(req)) ?? supabaseAdmin;
        const { data, error } = await rpcClient.rpc(rpcName, params);
        if (error) {
      ...
      if (name === "convene_council") {
        ...
        const produceInner = async (notify: ProgressFn) => {
          // Hand the client the run handle BEFORE the long work starts · on a
          // client timeout the final payload never arrives, but this does.
          notify(JSON.stringify({ run_id: runId, status: "running", poll_with: "council_minute_fetch", poll_args: { latest: true } }));
      ...
      if (name === "abe_weighing_in" || name === "summon_dissent") {
        ...
        const abeProduceInner = async (notify: ProgressFn) => {
          notify(JSON.stringify({ run_id: abeRunId, status: "running", poll_with: "council_minute_fetch", poll_args: { latest: true } }));
        ...
        const abeProduce = async (notify: ProgressFn) => {
          try {
            return await abeProduceInner(notify);
          } catch (e) {
            await failMinuteRun(supabaseAdmin, abeRunId, e);
            throw e;
          }
        };
        if (progressToken !== undefined) {
