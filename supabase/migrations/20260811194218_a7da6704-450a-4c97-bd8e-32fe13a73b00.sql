...
begin
  ...
  -- Found regardless of status. A closed signal reopens; it never forks.
  ...
  if v_id is null then
    -- Every register row carries a reference number. Allocating it here is
    -- what lets a signal be raised at all; without it the insert is refused.
    ...
    values (p_cid, public.next_curn(p_cid, 'signal'), v_key, p_key, 1, p_detail, coalesce(p_audience,'operator'),
