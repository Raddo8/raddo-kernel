-- ═══ HARDEN-10 · K2 · scheduled_actions gets a runner ══════════════════════
-- The finding was never "99 rows never fired". 53 fired and carry fired_at,
-- and the lease columns already exist. The narrow, true finding: the lease
-- model has no process that takes a lease. This is that process.

-- States that mean a human owns the row. The runner never touches these.
CREATE OR REPLACE FUNCTION public.scheduled_human_owned_states()
RETURNS text[] LANGUAGE sql IMMUTABLE
AS $$ SELECT ARRAY['jake_owned','gated_credits','held','parked']::text[] $$;

CREATE OR REPLACE FUNCTION public.scheduled_runnable_states()
RETURNS text[] LANGUAGE sql IMMUTABLE
AS $$ SELECT ARRAY['built','ready','scheduled','pending','due']::text[] $$;

-- Reclaim leases orphaned by a runner that died holding one.
CREATE OR REPLACE FUNCTION public.scheduled_reap_stale_claims()
RETURNS integer
  ...
  WITH r AS (
    UPDATE scheduled_actions
       SET claimed_at = NULL, claim_expires_at = NULL, claimed_by = NULL,
           last_error = coalesce(last_error, 'lease expired before the runner reported; reclaimed'),
           updated_at = now()
     WHERE claim_expires_at IS NOT NULL AND claim_expires_at < now()
       AND status = ANY (public.scheduled_runnable_states())
    RETURNING 1)
  SELECT count(*)::int FROM r;
$$;
REVOKE ALL ON FUNCTION public.scheduled_reap_stale_claims() FROM public, anon;
GRANT EXECUTE ON FUNCTION public.scheduled_reap_stale_claims() TO service_role;
...
BEGIN
  ...
  -- Claim under the existing lease. FOR UPDATE SKIP LOCKED is what makes two
  -- runners safe; the lease columns are what make a dead runner recoverable.
  FOR v_row IN
  ...
  LOOP
    ...
    BEGIN
      -- The effect: a scheduled action that comes due becomes work someone
      -- can see. A runner that fires without surfacing anything is a no-op
      -- with a timestamp on it.
      v_work := public.work_raise(
        v_row.cid, coalesce(v_row.title, v_row.program, 'scheduled action'),
        'task', 'scheduled_actions', 'scheduled_actions', v_row.id::text,
  ...
  -- What the runner MEASURED, not what it hopes. Zero eligible is a reason.
  ...
  RETURN jsonb_build_object(
    ...
    'reason', CASE WHEN v_claimed > 0 THEN NULL
      ...
      END);
END $function$;
REVOKE ALL ON FUNCTION public.scheduled_actions_run(text,integer,integer,integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.scheduled_actions_run(text,integer,integer,integer) TO service_role;