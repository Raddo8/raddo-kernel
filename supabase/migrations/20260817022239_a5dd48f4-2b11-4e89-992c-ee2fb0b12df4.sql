ALTER TABLE public.client_access_canary_result
  ADD COLUMN IF NOT EXISTS write_status text,
  ADD COLUMN IF NOT EXISTS write_detail text;
ALTER TABLE public.client_access_canary_run
  ADD COLUMN IF NOT EXISTS write_ok int;
...
BEGIN
  ...
  LOOP
    ...
    -- Harmless and reversible: re-assert the item's own principal_acts through
    -- work_dispose declaring the tenant, then restore the prior value.
    -- A tenant that can read and cannot write is a FAILED canary, not a pass.
    BEGIN
      ...
      ELSE
        EXECUTE 'select public.work_dispose($1,$2,null,$3,null,null,$4)'
          USING v_work, 'tracked', coalesce(v_prev,false), r.cid;
        UPDATE public.work_item
           SET principal_acts = v_prev, lane = v_prev_lane, updated_at = updated_at
         WHERE work_id = v_work;
        v_wr := 'OK';
        v_wr_d := 'work_dispose accepted a declared-tenant client write on work_id ' || v_work || ' · reverted';
      END IF;
    EXCEPTION WHEN others THEN
      v_wr := 'FAIL';
