-- HARDEN-15 Q1c fix · inside a SECURITY DEFINER function current_user is the
-- OWNER, so it cannot be used to recognise a privileged caller. Resolve the
-- caller from the JWT claim, with session_user as the direct-connection case.
...
AS $$
  SELECT CASE
    WHEN coalesce(nullif(current_setting('request.jwt.claims', true),'')::json->>'role','') = 'service_role'
      THEN true
    WHEN coalesce(nullif(current_setting('request.jwt.claims', true),'')::json->>'role','') <> ''
      THEN false
...
CREATE OR REPLACE FUNCTION public.work_dispose(
  ...
  p_lane text DEFAULT NULL::text, p_cid text DEFAULT NULL::text)
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
declare
  v_cid text; v_caller_cid text;
...
begin
  ...
  v_svc := public.caller_is_service_role();
